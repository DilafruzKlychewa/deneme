...
public class MergeSort {
    ...
    public static void merge(int arr[], int left, int middle, int right) {
        ...
        for (i = 0; i < low; i++) { //copy elemnts into left sub-array
            L[i] = arr[left + 1];
        }

        for (j = 0; j < high; j++) { //copy elemnts into right sub-array
        ...
        while (i < low && j < high) { // merge islemi yapildi. merge the left and right subarray
        ...
        while (j < high) { //merge the remaining elements from left subarray
